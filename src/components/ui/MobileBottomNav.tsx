...
export default function MobileBottomNav() {
  ...
  return (
    <nav className="lg:hidden fixed bottom-0 inset-x-0 z-50 h-16 bg-background/90 backdrop-blur-xl border-t border-border/60">
      <div className="h-full grid grid-cols-5">
        {LINKS.map(({ href, icon: Icon, label }) => {
          const active = pathname === href;
          const locked = !isSignedIn && ["/notifications", "/bookmarks", "/profile"].includes(href);
          if (locked) return <div key={href} />;
          return (
            <Link key={href} href={href}
              ...
              <motion.div whileTap={{ scale: 0.85 }} transition={{ type: "spring", stiffness: 400 }}>
                <Icon className={cn("size-5", active && "drop-shadow-[0_0_6px_oklch(0.67_0.22_264/0.8)]")} />
              </motion.div>
